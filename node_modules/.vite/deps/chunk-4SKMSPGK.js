...
// node_modules/@splinetool/runtime/build/runtime-chunk-EPHVY3XO.js
...
function O(t, n2, s3, o2, a2, h2) {
  let l2 = { Audio: [], Video: [], Particles: [], Link: [], Destroy: [], Create: [], Reset: [], Transition: [], TimelineAnimation: [], SwitchCamera: [], Animation: [], SceneTransition: [], SetVariable: [], DynamicVariablePlay: [], ClearLocalStorage: [], UserAPI: [] }, c2 = (n3) => ({ id: d2, data: p2 }) => {
    try {
      if ("Conditional" === p2.type) return void (Wa[t.type].includes("Conditional") && (p2.ifActions.forEach(c2({ expression: p2.condition, negate: false })), p2.elseActions.forEach(c2({ expression: p2.condition, negate: true }))));
      if (!Wa[t.type].includes(p2.type)) return;
      let u2 = (function(e) {
        return i.get(e);
      })(p2.type);
      if (void 0 === u2) return void s(x[p2.type] ?? "misc-actions", `a "${p2.type}" action`);
      let f2 = u2({ id: d2, data: p2, object: h2, page: s3, shared: o2, eventManager: a2, eventType: t.type, actionCondition: n3 });
      null != f2 && p2.type in l2 && l2[p2.type].push(f2);
    } catch (e) {
    }
  };
  return n2.forEach(c2()), l2;
}
function w(e) {
...
//# sourceMappingURL=chunk-4SKMSPGK.js.map
