import {
  p,
  v
} from "./chunk-GKWHISB2.js";
import {
  e as e2
} from "./chunk-53XVISTZ.js";
import {
  F,
  P,
  k
} from "./chunk-KTNZXI2Q.js";
import {
  V
} from "./chunk-NDLCMEPP.js";
import {
  i,
  s as s2
} from "./chunk-5REV3WFT.js";
import {
  ai,
  ci,
  di,
  ir,
  ui
} from "./chunk-EXQGW6OA.js";
import {
  $t,
  O,
  Pt,
  ct
} from "./chunk-XTM2MF4D.js";
import {
  e
} from "./chunk-PIDU3GFS.js";
import {
  h,
  s
} from "./chunk-EPYBTGE4.js";
import {
  Ar,
  Ot,
  Qr,
  xr
} from "./chunk-NCELNTZW.js";
import {
  As,
  Jo,
  Kr,
  Ll,
  Ra,
  Za,
  hr,
  nr,
  rr
} from "./chunk-G2UECJUH.js";

// node_modules/@splinetool/runtime/build/runtime-chunk-HZ6FL5GH.js
function U(e3) {
  return e3 instanceof ir && e3.needsAO && !e(e3) && true !== e3.isDecalObject;
}
function z(e3) {
  let t = e3.cloner;
  if (t && Array.isArray(t.children)) for (let a of t.children) (a == null ? void 0 : a.layers) && (a.layers.mask = e3.layers.mask);
}
function E(e3) {
  return e3.getLayersOfType("reflection").filter((e4) => e4.data.visible).length;
}
function P2(e3) {
  return e3.getLayersOfType("reflection").some((e4) => e4.data.visible && 1 === e4.data.mirror);
}
function x(e3) {
  let t = 1 / 0, a = -1 / 0;
  for (let i2 of e3.getLayersOfType("reflection")) {
    let e4 = i2.data;
    if (!e4.visible || 1 === e4.mirror) continue;
    let r = "number" == typeof e4.blur ? e4.blur : null;
    t = Math.min(t, r ?? 0), a = Math.max(a, r ?? 10);
  }
  return t === 1 / 0 ? null : { min: t, max: a };
}
function I(e3) {
  for (let t of e3.getLayersOfType("reflection")) {
    let e4 = t.data;
    if (!e4.visible) continue;
    let { blur: a, thickness: i2, steps: r, stepSize: s3 } = e4;
    return "number" != typeof a || "number" != typeof i2 || "number" != typeof r || "number" != typeof s3 ? "variable" : { blur: a, thickness: i2, steps: r, stepSize: s3, tracingMode: 1 === e4.tracingMode ? 1 : 0 };
  }
  return null;
}
function V2(e3, t) {
  let a = { outline: false, cavity: false }, i2 = 512 & e3.layers.mask, r = t.getLayersOfType("transmission"), s3 = t.getLayersOfType("outline"), n = E(t), h2 = t.getLayersOfType("cavity");
  return (s3.length > 0 || h2.length > 0) && (e3.layers.set(8), r.length > 0 && e3.layers.enable(3), n > 0 && e3.layers.enable(4), a.outline = s3.length > 0, a.cavity = h2.length > 0, s2(e3), i(e3)), 0 === r.length && 0 === s3.length && 0 === h2.length && 0 === n && e3.layers.set(0), i2 && e3.layers.set(9), !i2 && U(e3) && e3.layers.enable(5), z(e3), a;
}
function W(e3, t) {
  if (!t.layers) return false;
  let a = 512 & e3.layers.mask, i2 = false, r = t.getLayersOfType("transmission").filter((e4) => e4.data.visible), s3 = t.getLayersOfType("outline").filter((e4) => e4.data.visible), n = E(t), l = t.getLayersOfType("cavity").filter((e4) => e4.data.visible);
  return r.length > 0 && (e3.layers.set(3), (s3.length > 0 || l.length > 0) && e3.layers.enable(8), n > 0 && e3.layers.enable(4), i2 = true), 0 === r.length && 0 === s3.length && 0 === l.length && 0 === n && e3.layers.set(0), a && e3.layers.set(9), !a && U(e3) && e3.layers.enable(5), z(e3), i2;
}
function K(e3, t) {
  var a;
  if (!t.layers) return false;
  let i2 = 512 & e3.layers.mask, r = false, s3 = E(t), n = t.getLayersOfType("transmission").filter((e4) => e4.data.visible), l = t.getLayersOfType("outline").filter((e4) => e4.data.visible), o = t.getLayersOfType("cavity").filter((e4) => e4.data.visible);
  return s3 > 0 && ((a = e3.userData).__splinePlanarMirror ?? (a.__splinePlanarMirror = 0), e3.layers.set(4), n.length > 0 && e3.layers.enable(3), (l.length > 0 || o.length > 0) && e3.layers.enable(8), r = true), 0 === s3 && 0 === n.length && 0 === l.length && 0 === o.length && e3.layers.set(0), i2 && e3.layers.set(9), !i2 && U(e3) && e3.layers.enable(5), z(e3), r;
}
function _(e3) {
  let t = { outline: false, cavity: false }, a = (e4) => {
    t.outline = t.outline || e4.outline, t.cavity = t.cavity || e4.cavity;
  };
  return e3.traverseEntity((e4) => {
    if (e4 instanceof ir) if (Array.isArray(e4.material)) for (let t2 = 0; t2 < e4.material.length; t2++) a(V2(e4, e4.material[t2]));
    else a(V2(e4, e4.material));
  }), t;
}
function G(e3) {
  let t = false;
  return e3.traverseEntity((e4) => {
    if (e4 instanceof ir) if (Array.isArray(e4.material)) for (let a = 0; a < e4.material.length; a++) W(e4, e4.material[a]) && (t = true);
    else W(e4, e4.material) && (t = true);
  }), t;
}
function J(e3) {
  let t = false;
  return e3.traverseEntity((e4) => {
    if (e4 instanceof ir) if (Array.isArray(e4.material)) for (let a = 0; a < e4.material.length; a++) K(e4, e4.material[a]) && (t = true);
    else K(e4, e4.material) && (t = true);
  }), t;
}
var Z = new Ll();
var R = new As();
var N = new Ra();
var Y = new Kr();
function Q(e3, t, a, i2 = false) {
  let r = e3.cloner;
  if (r) {
    let s3 = null;
    if (e3.raycast === Za.prototype.raycast || e3.raycast === $t.prototype.raycast) {
      let t2 = e3.geometry;
      t2 && (null === t2.boundingSphere && t2.computeBoundingSphere(), s3 = t2.boundingSphere);
    }
    for (let n of r.children) {
      if (null !== s3 && (Y.copy(s3).applyMatrix4(n.matrixWorld), !t.ray.intersectsSphere(Y))) continue;
      let r2 = R.copy(n.matrixWorld).invert(), l = N.copy(t.ray).applyMatrix4(r2), o = e3.matrixWorld;
      l.applyMatrix4(o);
      let h2 = Z;
      h2.set(l.origin, l.direction), h2.near = t.near, h2.far = t.far, h2.layers.mask = t.layers.mask;
      let u = h2.intersectObject(e3, false);
      u.length > 0 && a.push({ ...u[0], object: i2 ? n : e3 });
    }
  }
}
var q = class extends Pt(hr) {
  constructor(e3, t, a) {
    if (super(), this.data = t, this.bgColor = new ci(1, 1, 1, 1), this.fog = null, this.backupFog = new nr(16777215, 0.1, 2e3), this.fogUseBGColor = false, this.fogMode = "linear", this.fogDensity = Ar.getDensity({}), this.fogHeightFalloff = Ar.getHeightFalloff({}), this.fogHeightBase = Ar.getHeightBase({}), this.isActive = false, this.skyState = null, this.skyStateVersion = 0, this.gradingState = null, this.skyHdriHolder = null, this.aoColor = new rr(), this.penumbraSizeArrayCache = null, this.super_Entity(e3, t), this.personalCamera = new V(Ot, { ...Qr.defaultData, ...t.camera, name: "Personal Camera" }), this.personalCamera.objectHelper.visible = false, this.add(this.personalCamera), this.activeCamera = this.personalCamera, this.ambientLight = new Jo(13882323, 8553090, 0.75 * Math.PI), this.ambientLight.name = "Default Ambient Light", this.ambientLight.layers.enable(3), this.ambientLight.removeFromParent(), this.add(this.ambientLight), t.uiScene) {
      let i2 = h();
      null === i2 ? s("ui-2d", "a page's 2D scene") : this.uiScene = new i2.Scene2D((Array.isArray(e3) ? e3[0] : e3) + "-ui", t.uiScene, { dpr: window.devicePixelRatio, shared: a.shared });
    } else t.uiFrame && this.createUICanvas(t.uiFrame, a);
  }
  get scene() {
    return this.parent;
  }
  get postprocessing() {
    return this.data.postprocessing;
  }
  get frameId() {
    return this.data.uiFrame;
  }
  get frame() {
    var _a;
    return (_a = this.uiCanvas) == null ? void 0 : _a.frame;
  }
  createUICanvas(e3, t) {
    let a = h();
    null !== a ? (this.uiCanvas = new a.UICanvas(this.uuid + "-uiscreen", window.innerWidth, window.innerHeight, t), this.uiCanvas.isScreenSpace = true, this.uiCanvas.promise.then(() => {
      var _a, _b;
      (_a = this.uiCanvas) == null ? void 0 : _a.applyFrame(e3), (_b = this.uiCanvas) == null ? void 0 : _b.applySize();
    })) : s("ui-2d", "a page's UI canvas");
  }
  updateVisible() {
  }
  setBackgroundColor(e3) {
    this.bgColor = e3, true === this.fogUseBGColor && (this.backupFog.color = e3);
  }
  entityChildrenCount() {
    return this.uiScene ? this.uiScene.children.length : super.entityChildrenCount();
  }
  updateAmbientLight(e3, t) {
    void 0 !== e3.color && (this.ambientLight.color = e2(e3.color, t)), void 0 !== e3.intensity && (this.ambientLight.intensity = e3.intensity * Math.PI), void 0 !== e3.enabled && (e3.enabled ? this.add(this.ambientLight) : this.remove(this.ambientLight));
  }
  onDeactive() {
    this.isActive = false;
  }
  onActive(e3) {
    this.isActive = true, e3.fog = this.fog, this.traverseEntity((e4) => {
      e4 instanceof P && e4.recomputeBoolean();
    }), this.updateShadow(this.data.shadow);
  }
  forceMaterialsUpdate() {
    this.traverseEntity((e3) => {
      if (e3 instanceof ir) if (Array.isArray(e3.material)) for (let t of e3.material) t.needsUpdate = true;
      else e3.material.needsUpdate = true, e3.material.dispose();
    });
  }
  updateShadow(e3) {
    if (void 0 === e3.softShadowQuality && void 0 === e3.type || !this.isActive) return;
    let t = this.data.shadow, a = xr.isSoft(t);
    ai(a, t == null ? void 0 : t.softShadowQuality) && this.forceMaterialsUpdate();
  }
  updateFog(e3, t) {
    e3.enabled ? this.fog = this.backupFog : this.fog = null, this.isActive && (this.scene.fog = this.fog), this.fogUseBGColor = e3.useBackgroundColor, e3.useBackgroundColor ? this.backupFog.color.set(this.bgColor) : this.backupFog.color = e2(e3.color, t), this.backupFog.near = e3.near, this.backupFog.far = e3.far, this.fogMode = Ar.getMode(e3), this.fogDensity = Ar.getDensity(e3), this.fogHeightFalloff = Ar.getHeightFalloff(e3), this.fogHeightBase = Ar.getHeightBase(e3);
  }
  updateSky(e3, t) {
    var _a;
    this.skyState = e3 ?? null, this.skyStateVersion++;
    let a = (_a = e3 == null ? void 0 : e3.hdri) == null ? void 0 : _a.image;
    this.skyHdriHolder = null != a ? t.image(a) : null;
  }
  updateAo(e3, t) {
    void 0 !== e3.aoColor && (this.aoColor = e2(e3.aoColor, t));
  }
  updateByOp(e3, t, a, i2) {
    let r = this.data.uiFrame;
    super.updateByOp(e3, t, a, i2);
    let s3 = t;
    O(e3.path, ["fog"]) ? this.updateFog(s3.fog, a.shared) : O(e3.path, ["sky"]) ? this.updateSky(s3.sky, a.shared) : O(e3.path, ["grading"]) ? this.gradingState = s3.grading ?? null : O(e3.path, ["ao"]) ? this.updateAo(s3.ao, a.shared) : O(e3.path, ["ambient"]) ? this.updateAmbientLight(s3.ambient, a.shared) : O(e3.path, ["shadow"]) ? this.updateShadow(s3.shadow) : 0 === e3.type && 0 === e3.path.length && (void 0 !== e3.props.uiFrame || r !== s3.uiFrame) && this.updateUIFrame(e3.props.uiFrame, a);
  }
  updateUIFrame(e3, t) {
    e3 ? (this.uiCanvas || this.createUICanvas(e3, t), this.uiCanvas.enabled = true, this.uiCanvas.applyFrame(e3)) : this.uiCanvas && (this.uiCanvas.enabled = false);
  }
  updateState(e3, t) {
    this.updateState_Entity(e3, t), void 0 !== e3.backgroundColor && this.setBackgroundColor(e2(e3.backgroundColor, t.shared)), void 0 !== e3.fog && this.updateFog(e3.fog, t.shared), void 0 !== e3.sky && this.updateSky(e3.sky, t.shared), void 0 !== e3.grading && (this.gradingState = e3.grading), void 0 !== e3.ambient && this.updateAmbientLight(e3.ambient, t.shared), void 0 !== e3.ao && this.updateAo(e3.ao, t.shared), void 0 !== e3.shadow && this.updateShadow(e3.shadow);
  }
  createFrame(e3, t) {
  }
  raycast(e3, t) {
    super.raycast(e3, t);
  }
  switchActiveCamera(e3) {
    e3 && e3.isDescendantOf(this) && (this.activeCamera !== this.personalCamera && (this.activeCamera.objectHelper.visible = true), this.activeCamera = e3, e3.objectHelper.visible = false);
  }
  get playCamera() {
    var _a;
    return ((_a = this.scene) == null ? void 0 : _a.find(this.data.publish.playCamera)) ?? this.personalCamera;
  }
  switchToPlayCamera() {
    this.switchActiveCamera(this.playCamera);
  }
  get penumbraSizeArray() {
    return null === this.penumbraSizeArrayCache && this.updatePenumbraSizeArray(), this.penumbraSizeArrayCache;
  }
  updatePenumbraSizeArray() {
    this.penumbraSizeArrayCache = new Array(di).fill(0.5);
    let a = 0, i2 = 0;
    this.traverseEntity((r) => {
      if (!r.visible) return true;
      r instanceof p && r.visible && a < ui && (this.penumbraSizeArrayCache[a] = r.data.penumbraSize ?? 0.5, a += 1), r instanceof v && r.visible && a < di - ui && (this.penumbraSizeArrayCache[ui + i2] = r.data.penumbraSize ?? 0.5, i2 += 1);
    });
  }
  raycastWithClones(e3) {
    let t = [], a = (s3) => {
      for (let n of s3.children) {
        let s4 = n.cloner;
        ct.is(n) && (n.visible || (s4 == null ? void 0 : s4.object.data.visible)) && ((F(n) || k(n) && this.scene.enableHelpers && n.objectHelper.visible) && (e3.intersectObject(n, false, t), Q(n, e3, t, true)), a(n));
      }
    };
    return a(this), t;
  }
  updateEntity2DByOp(e3, t, a, i2) {
    var _a;
    (_a = this.uiCanvas) == null ? void 0 : _a.updateEntity2DByOp(e3, t, a, i2);
  }
};

export {
  P2 as P,
  x,
  I,
  _,
  G,
  J,
  Q,
  q
};
//# sourceMappingURL=chunk-4YMXHKYL.js.map
